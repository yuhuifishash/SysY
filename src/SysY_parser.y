...
%token LEQ GEQ EQ NE // <=   >=   ==   != 
%token AND OR // &&    ||
%token CONST IF ELSE WHILE VOID_p INT FLOAT
...
VarDecl
:INT VarDef_list ';'{$$ = new VarDecl(1,$2);$$->set_line_number(line_number);}
|FLOAT VarDef_list ';'{$$ = new VarDecl(2,$2);$$->set_line_number(line_number);}
;

ConstDecl
:CONST INT ConstDef_list ';'{$$ = new ConstDecl(1,$3);$$->set_line_number(line_number);}
|CONST FLOAT ConstDef_list ';'{$$ = new ConstDecl(2,$3);$$->set_line_number(line_number);}
;
...
{$$ = new __FuncDef(1,$2,$4,$6);$$->set_line_number(line_number);}
|INT IDENT '(' ')' Block
{$$ = new __FuncDef(1,$2,new std::vector<FuncFParam>(),$5);$$->set_line_number(line_number);}
|FLOAT IDENT '(' FuncFParams ')' Block
{$$ = new __FuncDef(2,$2,$4,$6);$$->set_line_number(line_number);}
|FLOAT IDENT '(' ')' Block
{$$ = new __FuncDef(2,$2,new std::vector<FuncFParam>(),$5);$$->set_line_number(line_number);}
|VOID_p IDENT '(' FuncFParams ')' Block
{$$ = new __FuncDef(0,$2,$4,$6);$$->set_line_number(line_number);}
|VOID_p IDENT '(' ')' Block
{$$ = new __FuncDef(0,$2,new std::vector<FuncFParam>(),$5);$$->set_line_number(line_number);}
;
