...
bool NaturalLoop::LoopParallel(CFG *C, LLVMIR* IR) {
    ...
    auto brcondI = new BrCondInstruction(threadcmpI->GetResultReg(),GetNewLabelOperand(branchst->block_id),GetNewLabelOperand(branched->block_id));
    ...
    */
    ...
    if(depth < 4){
        ...
        auto condcmpI = new IcmpInstruction(I32,iterI->GetResultReg(),new ImmI32Operand(20),slt,GetNewRegOperand(++C->max_reg));
        Checkbb->InsertInstruction(1,condcmpI);
    }
    ...
    auto condbrI = new BrCondInstruction(GetNewRegOperand(C->max_reg),GetNewLabelOperand(header->block_id),GetNewLabelOperand(parallelbb->block_id));
