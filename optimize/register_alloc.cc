...
extern std::set<llvm_block> find_nodes_inloop(CFG* cfg,llvm_block n,llvm_block d);

std::map<llvm_block,int> get_inloop_nodes(CFG* cfg)
{
    std::map<llvm_block,int> loop_nodes;
...
void CFG::register_alloc()
{
    ...
    func_ins->set_ins_no(current_number++);

    int para_int_cnt = 0;
    int para_float_cnt = 0;
    for(auto formal_reg:func_ins->formals_reg){
        int formal_reg_no = ((reg_operand*)formal_reg)->getRegNo();
    ...
    while(!unnumberedBlockID.empty()){
        ...
        for(auto it = (*(block))[cur_BlockID]->Instruction_list.begin();it!=(*(block))[cur_BlockID]->Instruction_list.end();++it){
            auto instruction = *it;
            if(instruction->get_opcode() == FPTOSI){
                have_mem_f32 = 1;
            }
            int curinsNo = current_number;
            instruction->set_ins_no(current_number++);
            ...
            if(Spill_victim.options.no_compromise){
                ...
                if(physical_reg_type == 1){
                    (*block)[0]->Instruction_list.push_front(
                        new mov_instruction(
                            new reg_operand(Spill_victim.getVirtualRegNo()),
                            new reg_operand(RESERVED_R0+Spill_victim.options.prefer_regno)
                        )
                    );
                }else if(physical_reg_type == 2){
                    (*block)[0]->Instruction_list.push_front(
                        new vmov_instruction(
                            new reg_operand(Spill_victim.getVirtualRegNo()),
                            new reg_operand(RESERVED_S0+Spill_victim.options.prefer_regno)
                        )
                    );
                }
            }
...
std::vector<int> load_Instruction::refering_virtual_regs(){
    std::vector<int> ret;
    if(pointer->getOperandType()==basic_operand::REG){
        ret.push_back(
            ((reg_operand*)pointer)->getRegNo()
        );
    }
    if(result->getOperandType()==basic_operand::REG){
        ret.push_back(
            ((reg_operand*)result)->getRegNo()
        );
    }
    return ret;
}

std::vector<int> store_Instruction::refering_virtual_regs(){
    std::vector<int> ret;
    if(pointer->getOperandType()==basic_operand::REG){
        ret.push_back(
            ((reg_operand*)pointer)->getRegNo()
    ...
    if(value->getOperandType()==basic_operand::REG){
        ret.push_back(
            ((reg_operand*)value)->getRegNo()
        );
    }
    return ret;
}

std::vector<int> alg_Instruction::refering_virtual_regs(){
    std::vector<int> ret;
    if(op1->getOperandType()==basic_operand::REG){
        ret.push_back( ((reg_operand*)op1)->getRegNo());
    }
    if(op2->getOperandType()==basic_operand::REG){
        ret.push_back( ((reg_operand*)op2)->getRegNo());
    }
    if(result->getOperandType()==basic_operand::REG){
        ret.push_back(
            ((reg_operand*)result)->getRegNo()
...
std::vector<int> icmp_Instruction::refering_virtual_regs(){
    std::vector<int> ret;
    if(op1->getOperandType()==basic_operand::REG){
        ret.push_back( ((reg_operand*)op1)->getRegNo());
    }
...
std::vector<int> fcmp_Instruction::refering_virtual_regs(){
    std::vector<int> ret;
    if(op1->getOperandType()==basic_operand::REG){
        ret.push_back( ((reg_operand*)op1)->getRegNo());
    }
...
std::vector<int> phi_instruction::refering_virtual_regs(){
    ...
    for(auto var_label_pair:val_labels){
        ...
        if(val->getOperandType() == basic_operand::REG){
            ret.push_back(((reg_operand*)val)->getRegNo());
        }
    }
    return ret;
}
...
std::vector<int> call_instruction::refering_virtual_regs(){
    ...
    for(auto arg:args){
        if(arg.second->getOperandType()==basic_operand::REG){
            ret.push_back(((reg_operand*)arg.second)->getRegNo());
        }
    }
    return ret;
}
...
std::vector<int> ret_instruction::refering_virtual_regs(){
    ...
    if(ret_val != NULL){
        if(ret_val->getOperandType()==basic_operand::REG){
            ret.push_back(((reg_operand*)ret_val)->getRegNo());
        }
    }
    return ret;
}
...
std::vector<int> get_elementptr_instruction::refering_virtual_regs(){
    ...
    if(ptrval->getOperandType()==basic_operand::REG){
        ret.push_back(((reg_operand*)ptrval)->getRegNo());
    }
    for(auto idx:indexes){
        if(idx->getOperandType() == basic_operand::REG){
            ret.push_back(((reg_operand*)idx)->getRegNo());
        }
    }
    return ret;
}
...
std::vector<int> pseudo_alg_shift_Instruction::refering_virtual_regs(){
    ...
    if(op1->getOperandType()==basic_operand::REG){
        ret.push_back(
            ((reg_operand*)op1)->getRegNo()
        );
    }
    if(op2->getOperandType()==basic_operand::REG){
        ret.push_back(
            ((reg_operand*)op2)->getRegNo()
        );
    }
    return ret;
}
