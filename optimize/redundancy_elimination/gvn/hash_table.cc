...
static std::map<LLVMIROpcode, std::string> OpcodeMap = {{OTHER, "other"},
                                                        {LOAD, "load"},
                                                        {STORE, "store"},
                                                        {ADD, "add"},
                                                        {SUB, "sub"},
...
static std::map<FcmpCond, std::string> FcmpCondMap = {{FALSE, "FALSE"}, {OEQ, "OEQ"}, {OLT, "OLT"},  {OLE, "OLE"},
                                                      {ONE, "ONE"},     {ORD, "ORD"}, {TRUE, "TRUE"}};
...
int HashTable::lookupOrAddArithmetic(Instruction I) {
    ...
    if (opcode_ == LL_ADDMOD) {
        str = "call i32 @___llvm_ll_add_mod(i32 %v" + std::to_string(op1Value) + ",i32 %v" + std::to_string(op2Value) +
              ",i32 %v" + std::to_string(op3Value) + ")";
...
int HashTable::lookupOrAddIcmp(Instruction I) {
    ...
    std::string str = "icmp " + IcmpCondMap[cond_] + " " + TypeMap[type_] + " %v" + std::to_string(op1Value) + ",%v" +
                      std::to_string(op2Value);
...
int HashTable::lookupOrAddFcmp(Instruction I) {
    ...
    std::string str = "fcmp " + FcmpCondMap[cond_] + " " + TypeMap[type_] + " %v" + std::to_string(op1Value) + ",%v" +
                      std::to_string(op2Value);
