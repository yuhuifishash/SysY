...
Operand InlineCFG(CFG *uCFG,CFG *vCFG,LLVMBlock StartBB,LLVMBlock EndBB,std::map<int,int> &reg_replace_map,std::map<int,int> &label_replace_map){
...
void InlineCFG(CFG *uCFG,CFG *vCFG,uint32_t CallINo){
    ...
    for(auto formal:CallI->GetParameterList()){
        ...
        if(formal.second->GetOperandType()==BasicOperand::IMMI32){
            ...
            oldbb->InsertInstruction(0,new ArithmeticInstruction(ADD,I32,new ImmI32Operand(0),formal.second,newAddReg));
            while(!PhiISta.empty()){
            ...
            auto newAddReg=GetNewRegOperand(++uCFG->max_reg);
            oldbb->InsertInstruction(0,new ArithmeticInstruction(FADD,FLOAT32,new ImmF32Operand(0),formal.second,newAddReg));
            while(!PhiISta.empty()){
                oldbb->InsertInstruction(0,PhiISta.top());
                PhiISta.pop();
            }
...
void InlineDFS(CFG *uCFG){
    ...
    for(auto vCFG:fcallgraph.CG[uCFG]){
        ...
        InlineDFS(vCFG);
        if(fcallgraph.CG.find(vCFG)==fcallgraph.CG.end()||fcallgraph.CGNum[vCFG].find(vCFG)!=fcallgraph.CGNum[vCFG].end()){
            continue;
        }
