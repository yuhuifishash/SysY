...
void TailRecursiveEliminate(CFG* C)
{
    ...
    for(auto [id,bb]:*C->block_map){
        if(bb->Instruction_list.back()->GetOpcode() != RET){continue;}
        auto retI = (RetInstruction*)bb->Instruction_list.back();
        for(auto I:bb->Instruction_list){
            if(I->GetOpcode() != CALL){continue;}
            auto callI = (CallInstruction*)I;
            if(callI -> GetFunctionName() != FuncdefI->GetFunctionName()){continue;}
            ...
            if(opt1 || opt2){
                ...
                for(auto i = 0;i < list_size;i++,bb0_it--){
                    auto allocaI=*bb0_it;
                    if(callI->get_parameterList()[i].first==PTR){bb0_it++;continue;}
                    ...
                    if(callI_reg->GetRegNo() == i){continue;}//funtion params id stand by i
                    auto storeI=new StoreInstruction(callI->get_parameterList()[i].first,allocaI->GetResultReg(),callI->get_parameterList()[i].second);
