...
int in_allocas(std::set<int>& S,Instruction I)
{
    if(I->GetOpcode() == LOAD){
        auto LoadI = (LoadInstruction*)I;
        if(LoadI->GetPointer()->GetOperandType() != BasicOperand::REG){return -1;}
    ...
    if(I->GetOpcode() == STORE){
        auto StoreI = (StoreInstruction*)I;
        if(StoreI->GetPointer()->GetOperandType() != BasicOperand::REG){return -1;}
    ...
    while(!WorkList.empty()){
        ...
        for(auto &I:(*C->block_map)[BB]->Instruction_list){
            ...
            if(I->GetOpcode() == PHI){
                ...
                if(EraseSet.find(PhiI) != EraseSet.end()){continue;}
