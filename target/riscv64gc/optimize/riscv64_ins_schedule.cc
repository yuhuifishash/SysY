...
int MemOp[] = {RISCV_LB,  RISCV_LBU, RISCV_LH, RISCV_LHU, RISCV_LW, RISCV_LWU, RISCV_LD, RISCV_FLW,
               RISCV_FLD, RISCV_SB,  RISCV_SH, RISCV_SW,  RISCV_SD, RISCV_FSW, RISCV_FSD};

int BranchOp[] = {RISCV_BEQ,  RISCV_BNE, RISCV_BLT, RISCV_BGE,  RISCV_BLTU,
                  RISCV_BGEU, RISCV_BGT, RISCV_BLE, RISCV_BGTU, RISCV_BLEU};

int FaluOp[] = {
RISCV_FMV_W_X,   RISCV_FMV_X_W,   RISCV_FCVT_S_W,  RISCV_FCVT_D_W,  RISCV_FCVT_S_WU, RISCV_FCVT_D_WU, RISCV_FCVT_W_S,
RISCV_FCVT_W_D,  RISCV_FCVT_WU_S, RISCV_FCVT_WU_D, RISCV_FADD_S,    RISCV_FADD_D,    RISCV_FSUB_S,    RISCV_FSUB_D,
RISCV_FMUL_S,    RISCV_FMUL_D,    RISCV_FDIV_S,    RISCV_FDIV_D,    RISCV_FSQRT_S,   RISCV_FSQRT_D,   RISCV_FMADD_S,
RISCV_FMADD_D,   RISCV_FMSUB_S,   RISCV_FMSUB_D,   RISCV_FNMSUB_S,  RISCV_FNMSUB_D,  RISCV_FNMADD_S,  RISCV_FNMADD_D,
RISCV_FSGNJ_S,   RISCV_FSGNJ_D,   RISCV_FSGNJX_S,  RISCV_FSGNJX_D,  RISCV_FMIN_S,    RISCV_FMIN_D,    RISCV_FMAX_S,
RISCV_FMAX_D,    RISCV_FEQ_S,     RISCV_FEQ_D,     RISCV_FLT_S,     RISCV_FLT_D,     RISCV_FLE_S,     RISCV_FLE_D,
RISCV_FCLASS_S,  RISCV_FCLASS_D,  RISCV_FMV_D_X,   RISCV_FMV_X_D,   RISCV_FCVT_S_L,  RISCV_FCVT_D_L,  RISCV_FCVT_S_LU,
RISCV_FCVT_D_LU, RISCV_FCVT_L_S,  RISCV_FCVT_L_D,  RISCV_FCVT_LU_S, RISCV_FCVT_LU_D,
};
...
struct ExecState {
    ...
    bool full() {
        return (inflight.size() == 2) ||
               (inflight.size() == 1 && inflight.begin()->second != inflight.begin()->first->GetLatency());
    }
    ...
    bool Issue(MachineBaseInstruction *ins) {
        ...
        if (inflight.size() == 1 && inflight.begin()->second != inflight.begin()->first->GetLatency()) {
            return false;
        }
...
void RiscV64InstructionSchedule::ExecuteInList(std::vector<MachineBaseInstruction *> &list,
    ...
    for (auto ins : list) {
        ...
        if (ins->arch == MachineBaseInstruction::RiscV) {
            auto riscv_ins = (RiscV64Instruction *)(ins);
            if (riscv_ins->getOpcode() == RISCV_LB || riscv_ins->getOpcode() == RISCV_LBU ||
                riscv_ins->getOpcode() == RISCV_LH || riscv_ins->getOpcode() == RISCV_LHU ||
                riscv_ins->getOpcode() == RISCV_LW || riscv_ins->getOpcode() == RISCV_LWU ||
                riscv_ins->getOpcode() == RISCV_LD || riscv_ins->getOpcode() == RISCV_FLW ||
                riscv_ins->getOpcode() == RISCV_FLD) {
            ...
            if (riscv_ins->getOpcode() == RISCV_SB || riscv_ins->getOpcode() == RISCV_SH ||
                riscv_ins->getOpcode() == RISCV_SW || riscv_ins->getOpcode() == RISCV_SD ||
                riscv_ins->getOpcode() == RISCV_FSW || riscv_ins->getOpcode() == RISCV_FSD) {
                if (!last_loads.empty()) {
