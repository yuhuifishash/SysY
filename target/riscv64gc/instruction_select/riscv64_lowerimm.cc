        ...
        for (auto block : func->blocks) {
            ...
            for (auto it = block->begin(); it != block->end(); ++it) {
                ...
                if (cur_ins->arch == MachineBaseInstruction::RiscV) {
                        ...
                        if (reg2.is_virtual) {
                            if (IntConstPool.find(reg2.reg_no) != IntConstPool.end()) {
                                ...
                                if (cur_op == RISCV_MUL || cur_op == RISCV_MULW) {
                                    if (const_val == 1) {
                                        it = block->erase(it);
                                        block->insert(it,rvconstructor->ConstructCopyReg(cur_rvins->getRd(),reg1,INT64));
                                        --it;
                                    } else if (const_val == -1) {
                                        it = block->erase(it);
                                        auto op = RISCV_SUBW;
                                        ...
                                        block->insert(it,rvconstructor->ConstructR(op,cur_rvins->getRd(),GetPhysicalReg(RISCV_x0),reg1));
                                        --it;
                                        ...
                                        block->insert(it,rvconstructor->ConstructIImm(op,cur_rvins->getRd(),reg1,__builtin_ctz(const_val)));
                                        --it;
                                    }
                                }
                            }
                    ...
                    else if (cur_op == RISCV_DIV || cur_op == RISCV_DIVW || cur_op == RISCV_REM || cur_op == RISCV_REMW) {
                        auto reg1 = cur_rvins->getRs1();
                        auto reg2 = cur_rvins->getRs2();
                        if (reg2.is_virtual) {
                            if(IntConstPool.find(reg2.reg_no) != IntConstPool.end()) {
                                ...
                                if(cur_op == RISCV_DIV || cur_op == RISCV_DIVW) {
                                    ...
                                    if (negative) {
                                        block->insert(it,rvconstructor->ConstructR(RISCV_SUBW,cur_rvins->getRd(),GetPhysicalReg(RISCV_x0),result_reg));
                                    }
                                    --it;
                                    ...
                                    if(const_val == 1) {
                                        it = block->erase(it);
                                        block->insert(it,rvconstructor->ConstructCopyRegImmI(cur_rvins->getRd(),0,INT64));
                                        --it;
                                    } else if (const_val == 2) {
                                        it = block->erase(it);
                                        ...
                                        block->insert(it,rvconstructor->ConstructR(RISCV_SUBW,cur_rvins->getRd(),reg1,mid3_reg));
                                        --it;
                                    } else if (__builtin_popcount(const_val) == 1) {
                                        int log = __builtin_ctz(const_val);
                                        it = block->erase(it);
                                        ...
                                        block->insert(it,rvconstructor->ConstructR(RISCV_SUBW,cur_rvins->getRd(),reg1,mid4_reg));
                                        --it;
                                    } else {
                                        auto multiplier = chooseMultiplier(const_val,31);
                                        int mult = multiplier.m & (0xFFFFFFFF);
                                        ...
                                        if(false){
                                            ...
                                            block->insert(it,rvconstructor->ConstructR(RISCV_SUBW,result_reg,reg1,product_reg));
                                        }
                                        --it;
                                    }
                                }
