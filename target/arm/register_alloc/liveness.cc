...
void CFG::calculate_liveness(){
    ...
    for(auto reg:func_ins->formals_reg){
        if(reg->GetOperandType() == BasicOperand::REG){
            (*block)[0]->DEF.insert(((RegOperand*)reg)->GetRegNo());
        }else{
    ...
    for(auto block_pair:*block){
        ...
        for(auto ins:block->Instruction_list){
            ...
            auto candidate_def = ins->GetResultReg();
            if(candidate_def != NULL){
                if(candidate_def->GetOperandType() == BasicOperand::REG){
                    auto reg_def_no = ((RegOperand*)candidate_def)->GetRegNo();
            ...
            auto candidate_uses = ins->GetNonResultOperands();
            for(auto use:candidate_uses){
                if(use->GetOperandType() == BasicOperand::REG){
                    auto reg_use = ((RegOperand*)use);
