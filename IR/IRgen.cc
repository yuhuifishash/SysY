...
void AddExp_plus::codeIR()
{
    ...
    IRgenBinaryNode[addexp->attribute.T.type][mulexp->attribute.T.type]
                    (addexp,mulexp,NodeAttribute::ADD,B);
}
...
void AddExp_sub::codeIR()
{
    LLVMBlock B = llvmIR.GetBlock(function_now,now_label);
    IRgenBinaryNode[addexp->attribute.T.type][mulexp->attribute.T.type]
                    (addexp,mulexp,NodeAttribute::SUB,B);
}

void MulExp_mul::codeIR()
{
    LLVMBlock B = llvmIR.GetBlock(function_now,now_label);
    IRgenBinaryNode[mulexp->attribute.T.type][unary_exp->attribute.T.type]
                    (mulexp,unary_exp,NodeAttribute::MUL,B);
}
...
void MulExp_div::codeIR()
{
    LLVMBlock B = llvmIR.GetBlock(function_now,now_label);
    IRgenBinaryNode[mulexp->attribute.T.type][unary_exp->attribute.T.type]
                    (mulexp,unary_exp,NodeAttribute::DIV,B);
}
...
void MulExp_mod::codeIR()
{
    LLVMBlock B = llvmIR.GetBlock(function_now,now_label);
    IRgenBinaryNode[mulexp->attribute.T.type][unary_exp->attribute.T.type]
                    (mulexp,unary_exp,NodeAttribute::MOD,B);
}

void RelExp_leq::codeIR()
{
    LLVMBlock B = llvmIR.GetBlock(function_now,now_label);
    IRgenBinaryNode[relexp->attribute.T.type][addexp->attribute.T.type]
                    (relexp,addexp,NodeAttribute::LEQ,B);
}
...
void RelExp_lt::codeIR()
{
    LLVMBlock B = llvmIR.GetBlock(function_now,now_label);
    IRgenBinaryNode[relexp->attribute.T.type][addexp->attribute.T.type]
                    (relexp,addexp,NodeAttribute::LT,B);
}
...
void RelExp_geq::codeIR()
{
    LLVMBlock B = llvmIR.GetBlock(function_now,now_label);
    IRgenBinaryNode[relexp->attribute.T.type][addexp->attribute.T.type]
                    (relexp,addexp,NodeAttribute::GEQ,B);
}
...
void RelExp_gt::codeIR()
{
    LLVMBlock B = llvmIR.GetBlock(function_now,now_label);
    IRgenBinaryNode[relexp->attribute.T.type][addexp->attribute.T.type]
                    (relexp,addexp,NodeAttribute::GT,B);
}

void EqExp_eq::codeIR()
{
    LLVMBlock B = llvmIR.GetBlock(function_now,now_label);
    IRgenBinaryNode[eqexp->attribute.T.type][relexp->attribute.T.type]
                    (eqexp,relexp,NodeAttribute::EQ,B);
}
...
void EqExp_neq::codeIR()
{
    LLVMBlock B = llvmIR.GetBlock(function_now,now_label);
    IRgenBinaryNode[eqexp->attribute.T.type][relexp->attribute.T.type]
                    (eqexp,relexp,NodeAttribute::NE,B);
}
...
void Lval::codeIR()
{
    ...
    if(arrayindexs.empty() == false || attribute.T.type == Type::PTR){//lval is array, first use getelementptr to get address
...
void VarDecl::codeIR()
{
    ...
    for(auto def:def_vector){
        ...
        if(def->GetDims() != nullptr){//this var is array
            ...
            if(init != nullptr){
                int array_sz = 1;
                for(auto d:val.dims){array_sz *= d;}
...
void __FuncDef::codeIR()
{
    ...
    for(int i = 0;i < formal_vector.size(); ++i){
        ...
        if(formal->dims != nullptr){//formal is array
            ...
            for(int i = 1;i < formal->dims->size();++i){//in IRgen, we ignore the first dim of the formal
