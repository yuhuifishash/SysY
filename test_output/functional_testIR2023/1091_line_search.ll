declare i32 @getint()
declare i32 @getch()
declare float @getfloat()
declare i32 @getarray(ptr)
declare i32 @getfarray(ptr)
declare void @putint(i32)
declare void @putch(i32)
declare void @putfloat(float)
declare void @putarray(i32,ptr)
declare void @putfarray(i32,ptr)
declare void @putf(ptr)
declare void @_sysy_starttime(i32)
declare void @_sysy_stoptime(i32)
declare void @llvm.memset.p0.i32(ptr,i8,i32,i1)
define i32 @main()
{
L0:
    %r43 = alloca i32
    %r39 = alloca i32
    %r26 = alloca i32
    %r24 = alloca i32
    %r22 = alloca i32
    %r20 = alloca i32
    %r18 = alloca i32
    %r4 = alloca [10 x i32]
    %r2 = alloca i32
    %r0 = alloca i32
    br label %L1
L1:
    %r1 = add i32 0,0
    store i32 %r1, ptr %r0
    %r3 = add i32 0,0
    store i32 %r3, ptr %r2
    %r5 = add i32 0,0
    store i32 %r5, ptr %r2
    %r6 = add i32 0,0
    store i32 %r6, ptr %r0
    br label %L2
L2:
    %r7 = load i32, ptr %r0
    %r8 = add i32 10,0
    %r9 = icmp slt i32 %r7,%r8
    br i1 %r9, label %L3, label %L4
L3:
    %r10 = load i32, ptr %r0
    %r11 = getelementptr [10 x i32], ptr %r4, i32 0, i32 %r10
    %r12 = load i32, ptr %r0
    %r13 = add i32 1,0
    %r14 = add i32 %r12,%r13
    store i32 %r14, ptr %r11
    %r15 = load i32, ptr %r0
    %r16 = add i32 1,0
    %r17 = add i32 %r15,%r16
    store i32 %r17, ptr %r0
    br label %L2
L4:
    %r19 = add i32 0,0
    store i32 %r19, ptr %r18
    %r21 = add i32 0,0
    store i32 %r21, ptr %r20
    %r23 = add i32 0,0
    store i32 %r23, ptr %r22
    %r25 = add i32 0,0
    store i32 %r25, ptr %r24
    %r27 = add i32 0,0
    store i32 %r27, ptr %r26
    %r28 = add i32 10,0
    store i32 %r28, ptr %r26
    %r29 = call i32 @getint()
    store i32 %r29, ptr %r18
    %r30 = load i32, ptr %r26
    %r31 = add i32 1,0
    %r32 = sub i32 %r30,%r31
    store i32 %r32, ptr %r20
    %r33 = add i32 0,0
    store i32 %r33, ptr %r22
    %r34 = load i32, ptr %r20
    %r35 = load i32, ptr %r22
    %r36 = add i32 %r34,%r35
    %r37 = add i32 2,0
    %r38 = sdiv i32 %r36,%r37
    store i32 %r38, ptr %r24
    %r40 = add i32 0,0
    store i32 %r40, ptr %r39
    %r41 = add i32 0,0
    store i32 %r41, ptr %r39
    %r42 = add i32 0,0
    store i32 %r42, ptr %r0
    %r44 = add i32 0,0
    store i32 %r44, ptr %r43
    %r45 = add i32 0,0
    store i32 %r45, ptr %r43
    br label %L5
L5:
    %r46 = load i32, ptr %r0
    %r47 = add i32 10,0
    %r48 = icmp slt i32 %r46,%r47
    br i1 %r48, label %L8, label %L7
L6:
    %r52 = load i32, ptr %r0
    %r53 = getelementptr [10 x i32], ptr %r4, i32 0, i32 %r52
    %r54 = load i32, ptr %r53
    %r55 = load i32, ptr %r18
    %r56 = icmp eq i32 %r54,%r55
    br i1 %r56, label %L9, label %L10
L7:
    %r62 = load i32, ptr %r39
    %r63 = add i32 1,0
    %r64 = icmp eq i32 %r62,%r63
    br i1 %r64, label %L11, label %L12
L8:
    %r49 = load i32, ptr %r39
    %r50 = add i32 0,0
    %r51 = icmp eq i32 %r49,%r50
    br i1 %r51, label %L6, label %L7
L9:
    %r57 = add i32 1,0
    store i32 %r57, ptr %r39
    %r58 = load i32, ptr %r0
    store i32 %r58, ptr %r43
    br label %L10
L10:
    %r59 = load i32, ptr %r0
    %r60 = add i32 1,0
    %r61 = add i32 %r59,%r60
    store i32 %r61, ptr %r0
    br label %L5
L11:
    %r65 = load i32, ptr %r43
    call void @putint(i32 %r65)
    br label %L13
L12:
    %r66 = add i32 0,0
    store i32 %r66, ptr %r18
    %r67 = load i32, ptr %r18
    call void @putint(i32 %r67)
    br label %L13
L13:
    %r68 = add i32 10,0
    store i32 %r68, ptr %r18
    %r69 = load i32, ptr %r18
    call void @putch(i32 %r69)
    %r70 = add i32 0,0
    ret i32 %r70
}
